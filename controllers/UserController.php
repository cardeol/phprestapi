...
/**
 * @SWG\Info(title="MKD Api", version="0.1")
 */
...
$app->get('/users/search/{aa}', function (Request $request, Response $response) {
    
    $query = $request->getAttribute("query");
    $u = User::where('name', '=', $query)->get();
    Helper::Response($u);    
});
...
$app->post('/authenticate', function(Request $request, Response $response) {
    ...
    if($current_user !== null) {
        ...
        $token = UserToken::where("userid", $userid)
                ->whereDate('date_expiration','>',  Carbon::today()->toDateString())
                ->first();
         
        $jwt = "";
        if($token===null) {            
            $old_token = UserToken::where("userid",$userid)->first();            
            if($old_token!==null) {
                $old_token->delete();
            }
